...
class BaseBackbone(BaseModule, metaclass=ABCMeta):
    ...
    def deformable_grid_generate(self, saliency, rz_shape, device):
        ...
        uniform_grid_y, uniform_grid_x = torch.meshgrid(torch.arange(100, dtype=torch.float, device=device) + 0.5,
                                                        torch.arange(100, dtype=torch.float, device=device) + 0.5)
        ...
        uniform_grid_y_large, uniform_grid_x_large = torch.meshgrid(torch.arange(800, dtype=torch.float, device=device) + 0.5,
                                                                    torch.arange(800, dtype=torch.float, device=device) + 0.5)

        uniform_grid_large = torch.cat((2 * uniform_grid_x_large.unsqueeze(0) / 800 - 1,
                                  2 * uniform_grid_y_large.unsqueeze(0) / 800 - 1)).unsqueeze(0)
        ...
        return final_grid.permute(0, 2, 3, 1), uniform_grid_large.permute(0, 2, 3, 1), final_grid_large.permute(0, 2, 3, 1), final_grid_large_1600.permute(0, 2, 3, 1)

    def generate_interpolators(self, grid):
    ...
    def forward(self, x: torch.Tensor, zoomDet: bool = False) -> tuple:
        ...
        if zoomDet:
            for i, layer_name in enumerate(self.layers):
                ...
                if i == 2:
                    saliency_layer2 = self.offset_conv_layer2(x)
                    saliency = saliency_layer2
                    device = saliency.device
                    grid, uniform_grid_large, grid_large, grid_large_1600 = self.deformable_grid_generate(saliency, (800, 800), device)

                    x = F.grid_sample(x, grid, padding_mode='reflection')
                if i in self.out_indices:
                    outs.append(x)
            if self.training:
                interpolators = self.generate_interpolators(grid_large)

                return tuple(outs), grid_large, uniform_grid_large, interpolators

            else:
                interpolators = self.generate_interpolators(grid_large)
                interpolators_1600 = self.generate_interpolators(grid_large_1600)
                return tuple(outs), grid_large, interpolators, interpolators_1600, grid_large_1600
