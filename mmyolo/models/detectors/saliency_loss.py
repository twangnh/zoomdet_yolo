...
def saliency_loss(grid, szie_scaling, uniform_grid, ori_gt_bboxes, ori_shapes, saliency_weight, M, b, a, saliency_loss_mode='box_wise_loss'):
    ...
    for i in range(grid.shape[0]):
        ...
        area_scaling = szie_scaling**2
        small_indice = (x2 - x1) * (y2 - y1) <= (1024 / area_scaling)
        medium_indice = ((x2 - x1) * (y2 - y1) > (1024 / area_scaling)) & ((x2 - x1) * (y2 - y1) <= (9216 / area_scaling))
        large_indice = (x2 - x1) * (y2 - y1) > (9216 / area_scaling)
        ...
        if saliency_loss_mode == 'box_wise_loss':
            ...
            # uniform_saliency = F.interpolate(uniform_saliency,size=sampled_saliency.shape[2:], mode='bilinear', align_corners=False)

            if torch.any(small_indice):
                gt_mask_small = gt_mask[:,:,small_indice.squeeze().nonzero(as_tuple=True)[0]]
                # print(gt_mask_small.size(-1))
                gt_mask_small = gt_mask_small[:,:, :200]
                uniform_saliency_small = gt_mask_small.permute(2, 0, 1).unsqueeze(0)

                sampled_saliency = F.grid_sample(gt_mask_small.permute(2, 0, 1).unsqueeze(0), grid[i].unsqueeze(0),  padding_mode='reflection')
                sampled_saliency = F.interpolate(sampled_saliency, size=uniform_saliency_small.shape[2:], mode='bilinear', align_corners=False)

                sampled_point_count = sampled_saliency.sum(dim=(2, 3))
                uniform_point_count = uniform_saliency_small.sum(dim=(2, 3))
                del gt_mask_small
                del sampled_saliency
                ...
                mag_loss_s = torch_log_power(magnification, M=M, b=b, a=a)
