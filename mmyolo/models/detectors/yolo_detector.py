...
def compute_IOU(transformed_gt_bboxes, unwarp_gt_bboxes, test_cfg):
    ...
    test_cfg['area_sum'] += area_before.sum()
    test_cfg['area_small_sum'] += area_before[small_indice].sum()
    test_cfg['area_medium_sum'] += area_before[medium_indice].sum()
    test_cfg['area_large_sum'] += area_before[large_indice].sum()

    test_cfg['area_box_num_sum']+=len(area_before)
    test_cfg['area_box_num_sum_small']+=len(area_before[small_indice])
    test_cfg['area_box_num_sum_medium']+=len(area_before[medium_indice])
    test_cfg['area_box_num_sum_large']+=len(area_before[large_indice])
...
class SaliencyYOLODetector(SingleStageDetector):
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        ori_shape = tuple(batch_inputs.size()[2:])
        # rz_shape = (800, 800)
        ...
        self.len_gt.append(len(batch_transformed_gt_bboxes))
        ...
        saliency_losses = saliency_loss(grid, self.backbone.size_scaling, uniform_grid, batch_gt_bboxes,
    ...
    def predict(self,
        ...
        # torch.cuda.synchronize()
        # start_time = time_synchronized()
        x, grid, interpolators = self.extract_feat(batch_inputs)
        ...
        if len(batch_gt_bboxes)>0:
            ...
            unwarp_transformed_bboxes = unwarp_bboxes(batch_transformed_gt_bboxes, batch_input_shape, grid[0], batch_input_shape)
            ...
            compute_IOU(batch_gt_bboxes[:, 2:], unwarp_transformed_bboxes, self.test_cfg)
        ...
        results_list = self.bbox_head.predict(
            x, batch_input_shape, batch_data_samples, grid=grid, rescale=True)
