...
from mmyolo.models.detectors.yolo_detector import unwarp_bboxes
...
#         img, grid, align_corners=True, padding_mode='reflection').squeeze(0)
        ...
        for (bboxes, scores, objectness,
            ...
            if rescale:
                if grid is not None:
                    ...
                    results.bboxes = unwarp_bboxes(results.bboxes, batch_input_shape, grid[0], batch_input_shape)
                    # torch.cuda.synchronize()
                    # elapsed = time.perf_counter() - start_time
                    # print("unwarp time:", elapsed)
            ...
            # import cv2
            # img_id = batch_img_metas[0]['img_id']
            # ################### 可视化 ###########################
            # image = cv2.imread(batch_img_metas[0]['img_path'])
            # # 假设box_list是包含检测框的列表，每个框都是一个元组 (x1, y1, x2, y2)
            # box_list = results.bboxes.cpu().numpy()
            # # 遍历每个检测框并在图像上绘制
            # for box in box_list:
            #     x1, y1, x2, y2 = box
            ...
            # # 保存绘制好的图像到指定文件夹
            # output_path = '/root/autodl-tmp/baseline_DOTA_predict/img_ori_predict_{}.jpg'.format(img_id)
