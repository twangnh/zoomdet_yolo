...
class YOLOv5DetDataPreprocessor(DetDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        if isinstance(inputs, list):
            inputs = [input[[2, 1, 0], ...] for input in inputs]
            if self._enable_normalize:
                inputs = [(input - self.mean) / self.std for input in inputs]
        else:
        ...
        if isinstance(inputs, list):
            img_metas = [{'batch_input_shape': input.shape[1:]} for input in inputs]
        else:
            img_metas = [{'batch_input_shape': inputs.shape[2:]}] * len(inputs)
        data_samples_output = {
            'bboxes_labels': data_samples['bboxes_labels'],
            # 'ori_inputs': data_samples['ori_inputs'],
            # 'ori_gt_bboxes': data_samples['ori_gt_bboxes'],
            'img_metas': img_metas
        }
