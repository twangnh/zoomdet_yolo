...
class LetterResize(MMDET_Resize):
    ...
    def _resize_img(self, results: dict):
        ...
        if self.use_ori:
            ...
            max_length = max(image.shape[:2])
            min_length = min(image.shape[:2])
            top_padding = (max_length - min_length) / 2
            bottom_padding = (max_length - min_length) / 2
            left_padding = 0
            right_padding = 0
...
class LoadAnnotations(MMDET_LoadAnnotations):
    ...
    def _load_bboxes(self, results: dict):
        ...
        if self.box_type is None:
            ...
            results['ori_gt_bboxes'] = box_type_cls(gt_bboxes, dtype=torch.float32)
...
class YOLOv5RandomAffine(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if self.use_ori:
            border_shape = max(img.shape[0], img.shape[1])
            self.border = (-border_shape // 4, -border_shape // 4)
...
class RandomFlip(MMDET_RandomFlip):
    ...
    def _flip(self, results: dict) -> None:
        ...
        ori_img_shape = results['ori_inputs'].shape[:2]

        # filp ori images
        if results.get('ori_inputs', None) is not None:
            results['ori_inputs'] = mmcv.imflip(
            results['ori_inputs'], direction=results['flip_direction'])

        # flip ori gt bboxes
        if results.get('ori_gt_bboxes', None) is not None:
            results['ori_gt_bboxes'].flip_(ori_img_shape, results['flip_direction'])
