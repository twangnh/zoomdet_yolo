...
class YOLOv5ParamSchedulerHook(ParamSchedulerHook):
    ...
    def before_train(self, runner: Runner):
        ...
        """
        if isinstance(runner.optim_wrapper, OptimWrapperDict):
            self._base_lr = {}
            self._base_momentum = {}
            for name in runner.optim_wrapper.optim_wrappers:
                optimizer = runner.optim_wrapper.optim_wrappers[name].optimizer
                ...
                if cur_iters <= warmup_total_iters:
                    ...
                    for group_idx, param in enumerate(optimizer.param_groups):
                        if group_idx == 2:
                            ...
                            yp = [
                                self.warmup_bias_lr,
                                self._base_lr[name][group_idx] * self.scheduler_fn(cur_epoch)
                            ]
                        else:
                            yp = [
                                0.0,
                        ...
                        if 'momentum' in param:
                            param['momentum'] = np.interp(
                                cur_iters, xp,
                                [self.warmup_momentum, self._base_momentum[name][group_idx]])
                else:
    ...
    def after_train_epoch(self, runner: Runner):
        ...
        if isinstance(runner.optim_wrapper, OptimWrapperDict):
            for name in runner.optim_wrapper.optim_wrappers:
                optimizer = runner.optim_wrapper.optim_wrappers[name].optimizer
                for group_idx, param in enumerate(optimizer.param_groups):
                    param['lr'] = self._base_lr[name][group_idx] * self.scheduler_fn(
                        cur_epoch)
